...
class UserFilter:
    ...
    def classify_content_relevance(self):
        ...
        for user in self.total_user_dict:
            ...
            user['token'] = ' '.join([user['username'],
                                    user['description'] if user['description']\
                                          is not None else '',
                                    user['location'] if user['location'] is \
                                        not None else '',
                                    ' '.join(user['tweets']) if user['tweets']\
                                          is not None else ''])
            try:
                classification = pipe(user['token'], candidate_labels = \
                                      self.relevant_labels)
                relevant_labels_predicted = [label for label, score in \
                                             zip(classification["labels"],\
                ...
                print(f"Error classifying content relevance for user \
                      {user['username']}: {e}")
    ...
    def determine_location_relevance(self):
        ...
        for user in self.total_user_dict:
            if user['geo_location'] is not None and None not \
            in user['geo_location']:
            ...
            #latitude, longitude = user['geo_location']
                user_location = \
                    gpd.GeoDataFrame({'geometry': \
                                      [Point(user['geo_location'][1], \
                                             user['geo_location'][0])]}, \
                                                crs='EPSG:4326')
                
                shapefile_path = Path(__file__).\
                    parent.parent\
                          /"utils/shape_files/geoBoundaries-IND-ADM1-all/geoBoundaries-IND-ADM1_simplified.shp"
                
                shapefile = gpd.read_file(shapefile_path)
                joined_data = gpd.sjoin(user_location, shapefile, \
                                        how='left', op='within')
                ...
                desired_locations = constants.\
                    STATE_CAPITALS.get(self.location, [])
    ...
    def remove_users(self):
        ...
        self.filtered_user = []
        for user in self.total_user_dict:
            if user['location_relevance'] is True and \
            user['content_is_relevant'] is True:
    ...
    def run_filtering(self):
        ...
        try:
            ...
            print('users classified based on name,bio, \
