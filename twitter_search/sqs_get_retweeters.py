...
class UserRetweeters:
    def __init__(self, user_id, location, further_extraction, sqs_client, neptune_handler):
    ...
    async def twikit_get_single_tweet_retweeters(
        ...
        queue_url = self.sqs_client.get_queue_url(QueueName=SQS_USER_RETWEETERS)["QueueUrl"]
        retweeters_dict = {}
        ...
        if not flag:
            print(f'No retweeters extracted despite 3 retry attempts')
            return []
        ...
        for retweeter_dict in user_retweeters_list:
            if self.neptune_handler.user_exists(retweeter_dict["user_id"]):
                ...
                if (
                    ...
                    (retweeter_dict['city'] == retweeter_dict["target_location"]) and
                    (retweeter_dict["followers_count"] > INFLUENCER_FOLLOWERS_THRESHOLD)
                ):
                    root_users_counter += 1
                    self.send_to_queue(retweeter_dict['user_id'], SQS_USER_TWEETS)
                    self.send_to_queue(retweeter_dict['user_id'], SQS_USER_FOLLOWERS)
            ...
            self.neptune_handler.create_retweeter_edge(source_id=retweeter_dict['user_id'], 
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_retweeters", 
        type=int, 
        help="Number of retweeters to get"
    )
    parser.add_argument(
        "--extraction_type",
        type=str,
    ...
    while True:
        ...
        try:
            ...
            if tmp_user_id is not None:
                ...
                props_dict = {"retweeter_status": "completed",
                              "retweeter_last_processed": datetime.now(timezone.utc).isoformat()
                              }
                props_dict['last_updated'] = props_dict['retweeter_last_processed']
        ...
        if tmp_user_id != target_user_id:
            ...
            if tmp_user_id is not None:
                ...
                print(f"### Total tweets processed: {tweet_counter} ###")
            retweeter_status = neptune_handler.extract_node_attribute(label="User",
            ...
            if retweeter_status != "queued":
                print(f'Target user {target_user_id} not ready for retweeter extraction')
                continue
            ...
            print()
            print(f'Beginning retweeters extraction for User {user_counter} with ID {target_user_id}')
        ...
        user_retweeters.process_and_dispatch_retweeters(tweet_id, user_retweeters_list)
