...
from config_utils.constants import NEPTUNE_ENDPOINT, GREMLIN_ADD_INTERACTION
...
class NeptuneWriter:
    ...
    def add_interaction(self, source, source_username, source_followers,
        ...
        try:
            ...
            print(f"[AddInteraction] Error executing query: {e}. Reconnecting and retrying...")
            self._connect()
            retry_result = self.client.submitAsync(query).result().all().result()
            return retry_result
